...
//3.
app.get('/doctors/simple/:id', cache("10 minutes"),async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
    connection = await getOracleConnection(); // 👈 تعديل فقط
...
app.get("/students/by-university-id/:uniId", cache("10 minutes"),async (req, res) => {
...
app.get("/student_assignments/:studentId", cache("10 minutes"), async (req, res) => {
...
app.get("/check-patient/:patientUid", cache("10 minutes"),async (req, res) => {
...
app.get("/check-doctor/:id", cache("10 minutes"), async (req, res) => {
  const { id } = req.params;
  let connection;
...
app.get("/patients/by-appointment-id/:idnumber", cache("10 minutes"), async (req, res) => {
...
app.get("/pendingUsers", cache("10 minutes"), async (req, res) => {
  let connection;
  try {
    connection = await getOracleConnection(); // FIXED
...
app.get("/rejectedUsers", cache("10 minutes"), async (req, res) => {
  let connection;
  try {
    connection = await getOracleConnection(); // FIXED
...
app.get("/students/:userId", cache("10 minutes"), async (req, res) => {
  const { userId } = req.params;
...
app.get("/doctors/:id", cache("5 minu zates"), async (req, res) => {
  const { id } = req.params;
  let connection;
  try {
    connection = await getOracleConnection();
