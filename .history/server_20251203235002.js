...
app.post("/examinations", cache(10 cache("10 minutes")),auth, async (req, res) => {
  const {
    exam_id,
    patient_uid,
    doctor_id,
...
// ======================================================
app.get("/students", cache("20 seconds"), auth, async (req, res) => {
  let connection;
...
// ======================================================
app.get("/patients", cache("30 seconds"), auth, async (req, res) => {
  let connection;
...
app.get("/patients/by-appointment-id/:idnumber", cache("10 minutes"), auth, async (req, res) => {
...
app.get("/pendingUsers", cache("10 minutes"), auth, async (req, res) => {
  let connection;
  try {
    connection = await getOracleConnection(); // FIXED
...
app.get("/rejectedUsers", cache("10 minutes"), auth, async (req, res) => {
  let connection;
  try {
    connection = await getOracleConnection(); // FIXED
...
app.get("/all-examinations-full", cache("10 minutes"), auth, async (req, res) => {
  let connection;
  try {
    connection = await getOracleConnection();
