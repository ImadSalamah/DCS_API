...
app.get("/xray_requests", async (req, res) => {
  ...
  try {
    ...
    // ⭐ جلب parameter من query string إذا موجود
    const { status, doctorId } = req.query;

    let whereClause = "1=1"; // قاعدة افتراضية
    const binds = {};
    ...
    if (doctorId) {
      whereClause += " AND DOCTOR_UID = :doctor_uid";
    ...
    const sql = `
      SELECT 
        ...
        IMAGE as image
      FROM XRAY_REQUESTS
      WHERE ${whereClause}  // ⭐ استخدام الفلتر الديناميكي
