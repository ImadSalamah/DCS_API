...
// 36. Get all waiting list entries - FIXED NOTES FIELD
app.get("/waitingList", async (req, res) => {
  ...
  try {
    ...
    // 🔥 معالجة حقل NOTES بشكل صحيح
    const safeData = result.rows.map(row => {
      const cleanRow = { ...row };
      
      // معالجة حقل NOTES إذا كان يحتوي على [object Object]
      if (cleanRow.NOTES && cleanRow.NOTES === '[object Object]') {
        cleanRow.NOTES = { 
          error: 'Object was stringified incorrectly',
          original: '[object Object]'
        };
      }
      // إذا كان NOTES كائنًا حقيقيًا، احتفظ به كما هو
      else if (cleanRow.NOTES && typeof cleanRow.NOTES === 'object') {
        // لا تفعل شيئاً - احتفظ بالكائن كما هو
      }
      // إذا كان NOTES يحتوي على JSON صالح، حاول تحليله
      else if (cleanRow.NOTES && typeof cleanRow.NOTES === 'string') {
        try {
          // تحقق إذا كان يحتوي على JSON
          if (cleanRow.NOTES.trim().startsWith('{') || cleanRow.NOTES.trim().startsWith('[')) {
            cleanRow.NOTES = JSON.parse(cleanRow.NOTES);
          }
        } catch (e) {
          // إذا فشل التحليل، اتركه كسلسلة
          console.log('⚠️ Could not parse NOTES as JSON:', cleanRow.NOTES);
        }
      }
...
// 52. Get all pending x-ray requests
