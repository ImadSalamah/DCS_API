...
// 52. Get all x-ray requests (بعد التعديل)
app.get("/xray_requests", async (req, res) => {
  ...
  try {
    ...
    // ⭐ جلب parameters من query string
    const { status, doctorId } = req.query;

    // ⭐ بناء الشروط بشكل آمن
    const conditions = [];
    const binds = {};
    ...
    if (doctorId) {
      conditions.push("DOCTOR_UID = :doctor_uid");
      binds.doctor_uid = doctorId;
    }

    // ⭐ بناء WHERE clause بشكل آمن
    let whereClause = "";
    ...
    const sql = `
      SELECT 
        ...
        IMAGE as image
      FROM XRAY_REQUESTS
      ${whereClause}  // ⭐ استخدام الفلتر الديناميكي (قد يكون فارغاً)
      ORDER BY CREATED_AT DESC
    `;

    console.log("🔍 SQL Query:", sql); // ⭐ ديبيق للتحقق
    console.log("🔍 Binds:", binds);   // ⭐ ديبيق للتحقق
