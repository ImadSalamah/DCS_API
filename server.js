...
// these two lines for render line 35 and 36
const ORACLE_USER = process.env.DB_USER;
const ORACLE_PASSWORD = process.env.DB_PASS;
...
// ======================================================
app.get("/students", cache("2 minutes"), auth, async (req, res) => {
  let connection;
...
app.get("/students/by-university-id/:uniId", cache("10 minutes"), auth, async (req, res) => {
  const { uniId } = req.params;
  if (!uniId || uniId.toString().trim() === "") {
  ...
  try {
    connection = await getOracleConnection();
    const normalizedUniId = uniId.toString().trim();
    ...
    const result = await connection.execute(
      `SELECT 
       ...
       WHERE s.STUDENT_UNIVERSITY_ID = :uniId`,
    ...
    console.error("❌ Error fetching student by university ID:", err);
...
// ======================================================
// 6. Get all patients (Simple Version + Cache)
// ======================================================
app.get("/patients",  auth, async (req, res) => {
  ...
  // Pagination
  const page = parseInt(req.query.page || "1");
  const limit = parseInt(req.query.limit || "500"); // default = 500 حسب كودك الأصلي
  const offset = (page - 1) * limit;
    ...
    let sql = `
      ...
      FROM (
        SELECT 
          ...
          p.GRANDFATHERNAME as grandfatherName,
          ...
          p.STATUS as status,
          ROW_NUMBER() OVER (ORDER BY p.FIRSTNAME, p.FAMILYNAME) AS RN
        FROM PATIENTS p
        WHERE p.STATUS = 'active' 
           OR p.STATUS IS NULL 
           OR p.STATUS = 'EXAMINED'
      )
...
app.get("/student_assignments/:studentId", cache("10 minutes"), auth, async (req, res) => {
...
// 9. Update appointment examined status
app.put("/appointments/update_examined/:patientId", async (req, res) => {
  ...
  let connection;
  try {
    connection = await getOracleConnection(); // FIXED
...
//10. Check if patient exists
app.get("/check-patient/:patientUid", cache("10 minutes"), auth, async (req, res) => {
...
// 11. Check doctor data
app.get("/check-doctor/:id", cache("10 minutes"), auth, async (req, res) => {
  const { id } = req.params;
  let connection;
...
// 12. Find patient by appointment ID
app.get("/patients/by-appointment-id/:idnumber", auth, async (req, res) => {
...
// 20. Get student university ID (FIXED routing conflict)
app.get("/students/:userId", cache("10 minutes"), auth, async (req, res) => {
  const { userId } = req.params;
...
app.get("/users", cache("2 minutes"), auth, isAdmin, async (req, res) => {
  ...
  // Filter
  const usernameFilter = req.query.username?.toLowerCase() || null;
  ...
  try {
    ...
    // WHERE (optional)
    if (usernameFilter) {
      sql += ` WHERE LOWER(u.USERNAME) = :username `;
      binds.username = usernameFilter;
    }
...
// 22. 🔐 Add new user (Admins only) - IMPROVED
...
// 23. User CRUD operations - IMPROVED
app
  .route("/users/:id")
  // GET single user
...
app.post("/appointments", async (req, res) => {
  ...
  try {
    ...
    // ---- Server-side booking limit check ----
    // 1) get student's study year
    const stuRes = await connection.execute(
    ...
    if (stuRes.rows && stuRes.rows.length > 0) {
      studyYear = stuRes.rows[0].STUDY_YEAR || stuRes.rows[0].studyYear;
    }

    if (!studyYear) {
      return res.status(400).json({ message: '❌ Student study year not found' });
    }

    // 2) read booking limits
    const bsRes = await connection.execute(
      `SELECT FOURTH_YEAR_LIMIT, FIFTH_YEAR_LIMIT FROM BOOKING_SETTINGS`,
      [],
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );

    const fourthLimit = bsRes.rows[0] ? bsRes.rows[0].FOURTH_YEAR_LIMIT : 0;
    const fifthLimit = bsRes.rows[0] ? bsRes.rows[0].FIFTH_YEAR_LIMIT : 0;
    const limit = (Number(studyYear) === 4) ? fourthLimit : (Number(studyYear) === 5) ? fifthLimit : 0;

    if (!limit || limit <= 0) {
      return res.status(400).json({ message: '❌ Booking not allowed for this study year' });
    }

    // 3) count existing appointments for that study year on the date
    const countSql = `SELECT COUNT(*) AS COUNT FROM APPOINTMENTS a LEFT JOIN STUDENTS s ON a.STUDENT_ID = s.USER_ID WHERE TRUNC(a.APPOINTMENT_DATE) = TO_DATE(:p_date,'YYYY-MM-DD') AND s.STUDY_YEAR = :p_year`;
    const countBinds = { p_date: dateOnly, p_year: Number(studyYear) };
    const cntRes = await connection.execute(countSql, countBinds, { outFormat: oracledb.OUT_FORMAT_OBJECT });
    const currentCount = cntRes.rows && cntRes.rows[0] ? Number(cntRes.rows[0].COUNT) : 0;

    if (currentCount >= limit) {
      return res.status(400).json({ message: `❌ Day limit reached for study year ${studyYear}` });
    }
...
app.get("/appointments/count-by-year", auth, async (req, res) => {
  const { date, studyYear } = req.query;

  if (!date || !studyYear) {
    return res.status(400).json({ 
      error: "Missing date or studyYear parameter",
      message: "يجب أن تكون هناك معاملات: date و studyYear"
    });
  }
  ...
  try {
    connection = await getOracleConnection();
    // Use safe bind names and log query + binds for debugging
    const sql = `SELECT COUNT(*) AS COUNT
                 FROM APPOINTMENTS a
                 LEFT JOIN STUDENTS s ON a.STUDENT_ID = s.USER_ID
                 WHERE TRUNC(a.APPOINTMENT_DATE) = TO_DATE(:p_date, 'YYYY-MM-DD')
                 AND s.STUDY_YEAR = :p_year`;

    const binds = {
      p_date: date.split("T")[0],
      p_year: parseInt(studyYear)
    };

    console.debug('Executing SQL:', sql.replace(/\s+/g, ' '));
    console.debug('Binds:', binds);
    ...
    console.error("❌ Error fetching count by year:", err);
    res.status(500).json({ 
...
app.get("/appointments/count",  auth, async (req, res) => {
...
app.get("/waitingList",  auth, async (req, res) => {
  let connection;
  try {
    connection = await getOracleConnection();
    ...
    const result = await connection.execute(
      `SELECT 
        ...
        p.FAMILYNAME AS P_FAMILYNAME,
        p.MEDICAL_RECORD_NO AS P_MEDICAL_RECORD_NO

      FROM WAITING_LIST w
      LEFT JOIN PATIENTS p 
        ON w.PATIENT_UID = p.PATIENT_UID
    ...
    // 🔥 الحل النووي: بناء كائن جديد من الصفر
    const safeData = result.rows.map(row => {
      const cleanRow = {};
      Object.keys(row).forEach(key => {
        // تجاهل أي خصائص معقدة وتحويل القيم إلى أنواع بسيطة
        const value = row[key];
    ...
    console.error("❌ Error at GET /waitingList:", err);
    return res.status(500).json({
      success: false,
...
app.get("/all-examinations-simple", cache("10 minutes"), auth, async (req, res) => {
  let connection;
  try {
    connection = await getOracleConnection();
...
app.get("/examination-details/:examId", cache("10 minutes"), auth, async (req, res) => {
  let connection;
  try {
...
// 48. Get examination by patient ID 
app.get("/examinations/:patientId", cache("10 minutes"), auth, async (req, res) => {
  const { patientId } = req.params;
...
app.get("/all-examinations", cache("10 minutes"), auth, async (req, res) => {
  let connection;
  try {
    connection = await getOracleConnection();
...
app.get("/examination-full/:examId", cache("10 minutes"), auth, async (req, res) => {
  let connection;
  try {
    const { examId } = req.params;
    connection = await getOracleConnection();
...
// 62. NEW ENDPOINT: Add xray request
app.post("/xray_requests", async (req, res) => {
  const {
    ...
    doctorUid,
    image,
    status  // ⭐ إضافة status من الـ request body
  } = req.body;
  ...
  try {
    ...
    // ⭐ استخدام status من الـ request أو 'pending' كقيمة افتراضية
    ...
    const values = {
      ...
      status: finalStatus,  // ⭐ استخدام الحالة الديناميكية
      image: image || null
    };
    ...
    res.status(201).json({
      ...
      requestId,
      status: finalStatus,  // ⭐ إرجاع الحالة للمستخدم
      rowsAffected: result.rowsAffected
    });
...
// 63. UPDATE x-ray request (full update)
app.put("/xray_requests/:requestId", async (req, res) => {
  ...
  try {
    ...
    if (result.rowsAffected === 0) {
      return res.status(404).json({
        message: "❌ X-ray request not found"
      });
    }

    res.status(200).json({
      message: "✅ X-ray request updated successfully",
      requestId
    });

  } catch (err) {
    console.error("❌ Error updating x-ray request:", err);
    res.status(500).json({
      message: "❌ Error updating x-ray request",
      error: err.message
    });
  } finally {
    if (connection) await connection.close();
...
app.get("/xray_requests",auth, async (req, res) => {
  ...
  try {
    ...
    // 👈 ناخذ status من الـ query string لو موجود
    ...
    // 👈 لو في status نضيف where، لو ما في منجيب الكل
    const binds = {};
    if (status) {
...
// 57. Save xray image URL (image already uploaded to Oracle Object Storage)
app.post("/xray_images", async (req, res) => {
  ...
  try {
    ...
    // Get student name if student_id exists but student_name is not provided
    let finalStudentName = student_name;
    if (student_id && !finalStudentName) {
      const st = await connection.execute(
        `SELECT FULL_NAME FROM USERS WHERE USER_ID = :id`,
        { id: student_id },
        { outFormat: oracledb.OUT_FORMAT_OBJECT }
      );
      finalStudentName = st.rows[0]?.FULL_NAME || null;
    }
    ...
    const sql = `
      INSERT INTO XRAY_IMAGES (
        ...
        IMAGE_URL, UPLOADED_AT, UPLOADED_BY,
        STATUS, CAPTURED_AT, STUDY_YEAR, CLINIC
      ) VALUES (
        :img_id, :req, :pid, :pname,
        :sid, :sname, :type,
        :url, SYSTIMESTAMP, :uploaded_by,
        :status, TO_TIMESTAMP(:captured_at, 'YYYY-MM-DD"T"HH24:MI:SS.FF'), :study_year, :clinic
      )
    `;
    ...
    const bind = {
      ...
      sname: finalStudentName || fallback.STUDENT_NAME || null,
...
//58.
app.get("/xray-images/patient/:patientId", auth, async (req, res) => {
  const { patientId } = req.params;
  let connection;
...
//59.
app.get("/xray-images/request/:requestId", auth, async (req, res) => {
  const { requestId } = req.params;
  let connection;
...
//61.
app.get("/check-xray-requests", auth, async (req, res) => {
  let connection;
  try {
    connection = await getOracleConnection();
...
// 63. GET endpoint    
app.get('/student-xray-requests/:studentId', auth, async (req, res) => {
  const { studentId } = req.params;
...
// 64. Update xray image URL (Oracle)
...
app.get('/api/student-xray-requests/:studentId',  auth, async (req, res) => {
  const { studentId } = req.params;
...
app.get("/clinical_procedures/patient/:patientId",  auth, async (req, res) => {
  const { patientId } = req.params;
  let connection;
...
app.get("/prescriptions/patient/:patientId", auth, async (req, res) => {
  const { patientId } = req.params;
...
// 74. Assign patient to student (clean version)
  ...
  try {
    ...
    const query = `
      ...
      WHERE TRUNC(UPLOADED_AT) BETWEEN TO_DATE(:startDate, 'YYYY-MM-DD') AND TO_DATE(:endDate, 'YYYY-MM-DD')
        AND STATUS = 'completed'
      GROUP BY XRAY_TYPE, CLINIC, STUDY_YEAR
      ORDER BY XRAY_TYPE, CLINIC, STUDY_YEAR
    `;
    ...
    result.rows.forEach(row => {
      const [xrayType, clinic, studyYear, count] = row;
      const yearKey = `year_${studyYear}`;
...
app.get("/patients/check-id/:idNumber", auth, async (req, res) => {
  const { idNumber } = req.params;
...
app.put("/patients/:patientId", async (req, res) => {
  ...
  try {
    ...
    if (bind.idImage !== undefined) {
      bindDefs.idImage = {
        type: oracledb.CLOB,
        maxSize: Math.max(String(bind.idImage || "").length, 1)
      };
    }

    if (bind.iqrar !== undefined) {
      bindDefs.iqrar = {
        type: oracledb.CLOB,
        maxSize: Math.max(String(bind.iqrar || "").length, 1)
      };
    }

    if (Object.keys(bindDefs).length > 0) {
      executeOptions.bindDefs = bindDefs;
    }

    const result = await connection.execute(sql, bind, executeOptions);
...
async function getOracleConnection() {
  if (!global.oraclePool) {
    console.warn("⚠️ Pool not initialized, initializing now...");
    await initOraclePool();
  }
